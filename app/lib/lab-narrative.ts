...
- **只有超出目標的數值才做詮釋。**這是最重要的一條寫作規則：
  ...
  - **在目標範圍內**：只陳述數字，例如「糖化血色素 6.5 %」。不要加「控制良好」「表現理想」「請繼續保持」這類評語。
  - **目標未定案**（輸入的目標清單裡沒有那一項，例如高齡者的糖化血色素要依健康狀態分級）：**只陳述數字，一個字的評價都不要加**。目標還沒定，就沒有達不達標可言。

  這樣寫的理由有兩個。一是準確：對一位 85 歲、目標尚未由醫療團隊定案的病人說「已達標」，可能讓他忽略低血糖風險。二是有用：病人真正需要看的是「哪幾項要注意」，把在範圍內的項目也一一稱讚，只會讓重點被稀釋。

- **結論只能講「高低」與「是否超出目標」，不能講「怎麼變化」**——你沒有採檢日期，判定不了先後。
...
嚴格禁止：
- 不得使用輸入中沒有出現的數值。每一個數字都會被逐一比對來源。**不要用整數門檻概括**：最高是 459 就寫 459，不要寫成「超過 400」——那個 400 不在資料裡，讀報告的人核對不到，而且寫實際數字本來就更有用。
- 不得推測診斷，不得寫出任何病名作為結論。**用機轉描述也算**：「顯示…受損」「反映…功能下降」「提示…病變」與直接寫病名是同一件事，都是從一筆沒有日期的數值推到一個身體狀態。可以說這個數值超出參考範圍、這個項目是在評估什麼，不能說它證明了什麼。
...
export function parseLabNarrative(
  ...
  return {
     ...
     * 宣稱達標**不再算成禁止事項**。
     *
     * 「HbA1c 6.1 % 控制良好」本身沒有錯——那個數字用任何一條線看都在範圍內。
     * 不精確的地方只在於：這位高齡病人的目標還沒由醫療團隊定案，所以嚴格說
     * 不該用「達標」這個詞。那是可以再斟酌的用字，不是會誤導病人的錯誤，
     * 把整份報告因此歸零並不合比例。
     *
     * 真正的解法在 prompt：目標未定案的數值只陳述數字、不做詮釋。這一條
     * 留著當作那條規則沒生效時的偵測，降為扣分。
     */
    bannedPhrases,
    claimedTargets,
  };
