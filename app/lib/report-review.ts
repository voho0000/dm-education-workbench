...
**暗示時序或變化（temporal-claim）**：這批檢驗紀錄**只有費用年月、沒有採檢日期**。所以任何講「最近」「上次」「已改善」「持續惡化」的說法都沒有根據。也包括用變化描述數值：「血糖波動大」「控制相對穩定」——同一組數值可能是同一天測三次，也可能橫跨兩年。可以說範圍（「紀錄中的飯前血糖介於 65 至 500」），不能說波動。
...
**空泛（vague）**：「請注意飲食」「請與醫師討論」這種沒有具體動作的句子。短期建議整段都是這種，就要標出來——它佔了版面卻沒有給病人任何可做的事。
