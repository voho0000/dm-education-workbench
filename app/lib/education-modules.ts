...
export type EducationModule = {
   ...
   * 只給特定性別看的模組。
   *
   * 獨立審查抓到懷孕相關的眼科衛教被寫給一位男性病人。那不是錯的醫學內容，
   * 是**放錯人**——而讀報告的人分不出「這句不適用我」跟「這份報告不夠準」，
   * 兩者都會讓他不信任其餘內容。
   *
   * 性別未知時仍然納入：漏掉對育齡女性是安全上的損失，多給一段則只是雜訊。
   */
  sexGate?: { sex: "女性"; /** 已知年齡超過這個數就不給 */ maxAge: number };
...
2. 近期血糖變化較大時，請告知眼科與糖尿病照護團隊。`,
  ...
  },
  {
    ...
    topic: "R1",
    title: "懷孕與眼底檢查",
    appliesWhen: "已選 EYE-CORE，且性別為女性（或未知），且年齡未知或在 55 歲以下。",
    typeGate: "any",
    autoOnly: false,
    sexGate: { sex: "女性", maxAge: 55 },
    patientText:
      "懷孕期間視網膜病變可能加速變化。若您正在計畫懷孕或已經懷孕，請主動告知眼科與糖尿病照護團隊，以安排合適的檢查時程。",
  },
  {
    id: "EYE-T1",
    topic: "R1",
...
1. 曾有短暫單側無力、嘴歪、說話不清、突然視力異常或走路不穩，即使幾分鐘內就自己好了，也要當成緊急狀況立刻就醫評估——不要等到下次回診，也不要等它再發生一次。症狀消失不代表沒事。
...
3. 不要自行長期服用止痛藥或神經痛藥物。`,
     ...
     * 「突然」的神經症狀不放在這裡。
     *
     * 原文是「新出現明顯無力、走路突然不穩，或足部有傷口…：儘速就醫」。
     * 獨立審查三份都標為高：突發的無力與步態不穩本身就是中風警訊，卻被放在
     * 「儘速就醫」，還跟足部傷口併列——而同一份報告的腦血管段寫著同樣的症狀
     * 要「立即撥打 119」。同一個症狀在一份報告裡出現兩種急迫性，病人只能
     * 自己猜該聽哪一個。
     *
     * 神經病變要講的是**逐漸**的變化與足部傷口；突發的那一類交給腦血管段。
     */
    urgentSigns:
      "足部出現傷口、紅腫、化膿、發燒或明顯變色，或手腳無力、麻木在幾天內逐漸變明顯：儘速就醫。若是突然發生的單側無力、口齒不清或走路不穩，請依文末腦血管的做法立即撥打 119。",
    needsShared: ["foot"],
  },
  {
    id: "NERVE-T1",
